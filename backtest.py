...
def backtest(env, agent):
    ...
    return agent_r, equal_r, weights_df


# Results Table 

def print_results(agent_r, equal_r):
    ...
    metrics = {
        "Final Value ($1)"  : [agent_cum.iloc[-1],       equal_cum.iloc[-1]],
        "Ann. Return"       : [annual_return(agent_r),   annual_return(equal_r)],
        "Ann. Volatility"   : [annual_vol(agent_r),      annual_vol(equal_r)],
        "Sharpe Ratio"      : [sharpe(agent_r),           sharpe(equal_r)],
        "Max Drawdown"      : [max_drawdown(agent_cum),   max_drawdown(equal_cum)],
        "Hit Rate"          : [agent_hit, equal_hit],
    }

    print("\n" + "=" * 48)
    print(f"{'Metric':<22} {'Agent':>10} {'Equal Wt':>10}")
    print("-" * 48)
    for name, (a, e) in metrics.items():
        if "Value" in name or "Sharpe" in name:
            print(f"{name:<22} {a:>10.3f} {e:>10.3f}")
        else:
            print(f"{name:<22} {a:>10.1%} {e:>10.1%}")
    print("=" * 48)
...
def plot_results(agent_r, equal_r, weights_df, ticker_names=None):
    ...
    ax1.set_title("Cumulative Returns: Agent vs Equal Weight")
    ...
    ax3.set_title(f"Rolling{window}- Day Hit Rate")
    ...
    plt.suptitle("PPO Portfolio Agent — Backtest Results", fontsize=14, fontweight="bold")
    plt.show()
